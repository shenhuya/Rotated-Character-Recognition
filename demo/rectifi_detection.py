...
def rot_img_360(dets, im, angle, img):
    ...
    M[0, 2] += (new_w - im_w) / 2
    M[1, 2] += (new_h - im_h) / 2
    image_rotation = cv2.warpAffine(src=im, M=M, dsize=(new_w, new_h), borderValue=(means[0][0], means[1][0], means[2][0]))
    ...
    # print("dets-rot: ", dets)
    cv2.imwrite('/data1/hzj/mmrotate/data/rectify_img_test/' + osp.basename(img),image_rotation)
...
# 预测角度范围为0-360°
config_file_1= '/data1/hzj/mmrotate/work_dirs/work_dirs/360test-refine-final-flip/res18_kfiou_ln_r50_fpn_1x_GangPi_360.py'
...
checkpoint_file_1 = '/data1/hzj/mmrotate/work_dirs/work_dirs/360test-refine-final-flip/best_acc.pth'
...
# 包含整体字符朝向
# config_file_1 = '/data1/hzj/mmrotate/work_dirs/work_dirs/overall-orientation/res18_kfiou_ln_r50_fpn_1x_GangPi_360.py'
# checkpoint_file_1 = '/data1/hzj/mmrotate/work_dirs/work_dirs/overall-orientation/best_acc.pth'
...
model_1 = init_detector(config_file_1, checkpoint_file_1, device='cuda:0',  cfg_options={'model.test_cfg.score_thr':0.5})
...
save_path = '/data1/hzj/mmrotate/show-dir/overall-orientation+detection'
...
if __name__ == '__main__':
    ...
    for i, img in enumerate(img_names):
        ...
        # 获取矫正后的图像
        im_new = os.path.join('/data1/hzj/mmrotate/data/rectify_img_test/' + osp.basename(img))
        # 旋转后的图像进行检测识别
        result = inference_detector(model, im_new)
        labels = [
        ...
        model.show_result(os.path.join('/data1/hzj/mmrotate/data/rectify_img_test/' + osp.basename(img)), result, out_file=os.path.join(save_path, os.path.basename(img)), bbox_color=(0, 255, 255))
    
    end = time.time()
    print(acc_count)
    print(num_images)
    print(acc_count/(num_images) * 100, '%')
    print((end - start)/(num_images), 's')
