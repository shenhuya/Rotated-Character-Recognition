...
def multiclass_nms_rotated_landm(multi_bboxes,
                           multi_landms,
                           multi_scores,
                           score_thr,
                           nms,
                           ...
                           score_factors=None,
                           return_inds=False):
    """NMS for multi-class bboxes and landms.

    Args:
        multi_bboxes (torch.Tensor): shape (n, #class*5) or (n, 5)
        multi_landms (torch.Tensor): shape (n, #class*6) or (n, 6)
        multi_scores (torch.Tensor): shape (n, #class), where the last column
            contains scores of the background class, but this will be ignored.
        score_thr (float): bbox threshold, bboxes with scores lower than it
            will not be considered.
    ...
    Returns:
        tuple (dets, labels, indices (optional)): tensors of shape (k, 11), \
        (k), and (k). Dets are boxes with scores. Labels are 0-based.
    """
    num_classes = multi_scores.size(1) - 1
    # exclude background category
    if multi_bboxes.shape[1] > 5:
        ...
        bboxes = multi_bboxes[:, None].expand(
            multi_scores.size(0), num_classes, 5)
    if multi_landms.shape[1] > 6:
        landms = multi_landms.view(multi_scores.size(0), -1, 6)
    else:
        landms = multi_landms[:, None].expand(
    ...
    inds = valid_mask.nonzero(as_tuple=False).squeeze(1)
    bboxes, landms, scores, labels = bboxes[inds], landms[inds], scores[inds], labels[inds]

    if bboxes.numel() == 0:
        dets = torch.cat([bboxes, scores[:, None]], -1)
        if return_inds:
            return torch.cat([bboxes, landms], 1), labels, inds
        else:
            return torch.cat([bboxes, landms], 1), labels
    ...
    if return_inds:
        return torch.cat([bboxes, landms, scores[:, None]], 1), labels, keep
    else:
        return torch.cat([bboxes, landms, scores[:, None]], 1), labels
